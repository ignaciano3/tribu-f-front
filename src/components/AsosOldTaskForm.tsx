...
export default function AsosOldTaskForm(props: any) {
  ...
  console.log("idTicket en AsosOldTaskForm: ", idTicket);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await LinkTicketWithTask(idTicket, asosData.id_task);
      window.location.href = "/tickets/" + idTicket;
    } catch (error) {
      console.error(
        "Error al linkear un ticket con una tarea existente:",
        error
      );
    }
    // Aquí puedes manejar la lógica para enviar los datos del proyecto
    console.log("Datos de la asociacion:", asosData);
  };
  return (
    <>
